...
from playlist_etl.rapid_api_client import RapidAPIClient, get_json_response
...
class TestGetJsonResponse:
    ...
    @patch("playlist_etl.rapid_api_client.requests.get")
    def test_get_json_response_success(self, mock_get: Mock) -> None:
    ...
    @patch("playlist_etl.rapid_api_client.requests.get")
    def test_get_json_response_http_error(self, mock_get: Mock) -> None:
    ...
    @patch("playlist_etl.rapid_api_client.DEBUG_MODE", True)
    def test_get_json_response_debug_mode(self) -> None:
        """Test debug mode returns empty dict"""
        result = get_json_response(url="https://api.example.com/playlist", host="api.example.com", api_key="test_key")

        assert result == {}

    @patch("playlist_etl.rapid_api_client.NO_RAPID", True)
    def test_get_json_response_no_rapid_mode(self) -> None:
...
class TestExtractor:
    ...
    def test_extractor_init_spotify(self) -> None:
        """Test Extractor initialization for Spotify"""
        extractor = SpotifyFetcher("Spotify", "dance")
    ...
    def test_extractor_init_apple_music(self) -> None:
        """Test Extractor initialization for Apple Music"""
        extractor = AppleMusicFetcher("AppleMusic", "pop")
    ...
    def test_extractor_init_soundcloud(self) -> None:
        """Test Extractor initialization for SoundCloud"""
        extractor = SoundCloudFetcher("SoundCloud", "rap")
...
class TestServiceConfigurations:
    ...
    def test_service_config_completeness(self, service_name: str) -> None:
...
class TestPlaylistGenres:
    ...
    def test_playlist_genres_defined(self) -> None:
    ...
    def test_playlist_genres_match_models(self) -> None:
...
class TestRealWorldScenarios:
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_spotify_playlist_extraction(self, mock_get_json: Mock) -> None:
        """Test realistic Spotify playlist extraction"""
        # Mock realistic Spotify API response
        spotify_api_response: dict[str, Any] = {
            "items": [
                {
                    "track": {
                        "name": "As It Was",
        ...
        assert result == spotify_api_response
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_apple_music_playlist_extraction(self, mock_get_json: Mock) -> None:
        """Test realistic Apple Music playlist extraction"""
        # Mock realistic Apple Music API response
        apple_api_response: dict[str, Any] = {
            "album_details": {
                "0": {
                    "name": "Talk To Me",
        ...
        extractor = AppleMusicFetcher("AppleMusic", "dance")

        result = extractor.get_playlist()

        assert result == apple_api_response
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_soundcloud_playlist_extraction(self, mock_get_json: Mock) -> None:
        """Test realistic SoundCloud playlist extraction"""
        # Mock realistic SoundCloud API response
        soundcloud_api_response: dict[str, Any] = {
            "tracks": {
                "items": [
                    {
        ...
        mock_get_json.return_value = soundcloud_api_response

        extractor = SoundCloudFetcher("SoundCloud", "dance")

        result = extractor.get_playlist()

        assert result == soundcloud_api_response
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_extraction_with_api_error(self, mock_get_json: Mock) -> None:
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_extraction_with_empty_response(self, mock_get_json: Mock) -> None:
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_all_service_genre_combinations(self, mock_get_json: Mock, service_name: str, genre: str) -> None:
        ...
        extractor = fetcher_class(service_name, genre)
        result = extractor.get_playlist()

        assert result == {"test": "data"}
        assert mock_get_json.called

    def test_url_construction_spotify(self) -> None:
    ...
    def test_url_construction_apple_music(self) -> None:
    ...
    def test_url_construction_soundcloud(self) -> None:
...
class TestErrorHandling:
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_network_timeout_handling(self, mock_get_json: Mock) -> None:
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_invalid_json_response(self, mock_get_json: Mock) -> None:
    ...
    @patch("playlist_etl.rapid_api_client.get_json_response")
    def test_rate_limit_handling(self, mock_get_json: Mock) -> None:
...
class TestRunExtraction:
    ...
    def test_run_extraction_spotify_end_to_end(
        self, mock_webdriver: Mock, mock_requests_get: Mock, mock_get_json: Mock, mock_mongo_insert: Mock
    ) -> None:
        ...
        # Run the extraction
        run_extraction(self.mock_mongo_client, "Spotify", "dance")
    ...
    def test_run_extraction_apple_music_end_to_end(
        ...
        # Run the extraction
        run_extraction(self.mock_mongo_client, "AppleMusic", "pop")
    ...
    def test_run_extraction_debug_mode_skips_mongo(
        self, mock_requests_get: Mock, mock_get_json: Mock, mock_mongo_insert: Mock
    ) -> None:
    ...
    def test_run_extraction_invalid_service_raises_error(self) -> None:
        """Test that invalid service name raises ValueError"""
        with pytest.raises(ValueError, match="Unknown service: InvalidService"):
            run_extraction(self.mock_mongo_client, "InvalidService", "dance")
