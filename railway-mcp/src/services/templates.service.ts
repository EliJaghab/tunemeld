...
export class TemplatesService extends BaseService {
              ...
              .map(template => {
                const services = Object.entries(template.serializedConfig.services)
                  .map(
                    ...
                    Networking: ${service.networking?.tcpProxies ? "TCP Proxy enabled" : "No TCP Proxy"}, ${
                      Object.keys(service.networking?.serviceDomains || {}).length
                    } domains
  ...
  async deployTemplate(projectId: string, templateId: string, environmentId: string, teamId?: string) {
    try {
      // Get the template
