...
export class DomainService extends BaseService {
  ...
  async createServiceDomain(input: ServiceDomainCreateInput): Promise<CallToolResult> {
    try {
      ...
      return createSuccessResponse({
        text: `Domain created successfully: ${domain.domain} (ID: ${domain.id}, Port: ${
          domain.targetPort || "default"
        })`,
        data: domain,
      });
    } catch (error) {
   ...
   */
  async listDomains(projectId: string, environmentId: string, serviceId: string): Promise<CallToolResult> {
    try {
