...
${
  environments > 0
    ? `${project.environments.edges.map(edge => `- ${edge.node.name} (${edge.node.id})`).join("\n")}`
    : ""
}
Services: ${services}
...
Created: ${new Date(project.createdAt).toLocaleString()}`;
  ...
  async getProject(projectId: string): Promise<CallToolResult> {
    try {
      ...
      const environmentList = environments.map(env => `  🌍 ${env.name} (ID: ${env.id})`).join("\n");
...
${serviceList || "  No services"}`;
  ...
  async listEnvironments(projectId: string) {
    try {
      ...
      const environmentDetails = environments.map(
        env =>
          `🌍 ${env.name} (ID: ${env.id})
