...
export const deploymentTools = [
  createTool(
    ...
    }),
    {
      ...
      environmentId: z
        .string()
        .describe("ID of the environment to list deployments from (usually obtained from service_list)"),
  ...
  createTool(
    ...
    formatToolDescription({
      ...
      bestFor: ["Deploying code changes", "Applying configuration updates", "Rolling back to previous states"],
      notFor: [
  ...
  createTool(
    ...
    formatToolDescription({
      ...
      bestFor: ["Debugging deployment issues", "Monitoring deployment progress", "Checking build output"],
