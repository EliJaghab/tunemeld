...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Get today's records that don't have daily_view_change computed yet
        todays_records = HistoricalTrackViewCount.objects.filter(
            recorded_date=today,
            daily_view_change__isnull=True
        )
        ...
        for record in todays_records:
            ...
            if yesterday_record:
                ...
                logger.info(
                    f"{record.isrc} on {record.service.name}: "
                    f"{record.current_view_count:,} views (first record)"
                )
                updated_count += 1
