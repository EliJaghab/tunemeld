...
class Command(BaseCommand):
    ...
    def find_duplicate_isrcs(self):
        ...
        duplicates = Track.objects.values("isrc").annotate(count=Count("isrc")).filter(count__gt=1)
    ...
    def validate_unique_isrcs(self):
        ...
        if total_tracks == unique_count:
            ...
            logger.error(
                f"✗ VALIDATION FAILED: {total_tracks} tracks but only {unique_count} unique ISRCs "
                f"({duplicates} duplicates remain)"
            )
