...
func Transform(soundCloudTracks, spotifyTracks, appleMusicTracks []config.Track) ([]config.Track, error) {
	...
	compareAndPopulate(&foundTracks, soundCloudTracks, spotifyTracks, config.SourceSoundCloud, config.SourceSpotify, trackIndex)

	// Step 2: Compare SC and AP, update foundTracks accordingly
	compareAndPopulate(&foundTracks, soundCloudTracks, appleMusicTracks, config.SourceSoundCloud, config.SourceAppleMusic, trackIndex)

	// Step 3: For SP tracks not already processed, compare with AP and update foundTracks or add new entries
	compareAndPopulate(&foundTracks, spotifyTracks, appleMusicTracks, config.SourceSpotify, config.SourceAppleMusic, trackIndex)
...
func compareAndPopulate(foundTracks *[]config.Track, tracksA, tracksB []config.Track, sourceA, sourceB config.TrackSource, trackIndex map[string]int) {
		...
		for _, trackB := range tracksB {
			if fuzzyMatch(trackA, trackB) {
				...
				if index, exists := trackIndex[identifier]; exists {
					...
					// Inside the if exists block, add a condition to prioritize the ranking
					if exists {
						// Prioritize SoundCloud ranking, then Apple Music, then Spotify
						currentSource := (*foundTracks)[index].Source
						if currentSource == config.SourceSpotify && (sourceA == config.SourceSoundCloud || sourceA == config.SourceAppleMusic) || currentSource == config.SourceAppleMusic && sourceA == config.SourceSoundCloud {
					...
					*foundTracks = append(*foundTracks, trackA)
					...
					log.Printf("New track added: %s by %s with sources %s and %s", trackA.Name, trackA.Artist, sourceA.String(), sourceB.String())
				}
			}
		}
	}
...
func fuzzyMatch(trackA, trackB config.Track) bool {
	...
	if isMatch {
		log.Printf("Fuzzy matching \"%s\" by \"%s\" with \"%s\" by \"%s\": Match", trackA.Name, trackA.Artist, trackB.Name, trackB.Artist)
	}
	return isMatch
}
