...
function showHeaderArtError(message) {
  // Hide skeleton loaders first
  hideSkeletonLoaders();

  // Display error message to users
  const services = ["soundcloud", "applemusic", "spotify"];
  services.forEach(service => {
    const imagePlaceholder = document.getElementById(`${service}-image-placeholder`);
    const descriptionElement = document.getElementById(`${service}-description`);
    const titleElement = document.getElementById(`${service}-playlist-title`);

    if (imagePlaceholder) {
      imagePlaceholder.style.backgroundImage = "";
      imagePlaceholder.style.backgroundColor = "#f0f0f0";
      imagePlaceholder.innerHTML =
        '<div style="padding: 20px; text-align: center; color: #666;">Unable to load playlist art</div>';
    }

    if (titleElement) {
      titleElement.textContent = "Error loading playlist";
      titleElement.href = "#";
    }

    if (descriptionElement) {
      descriptionElement.textContent = `Sorry, we couldn't load the playlist information. ${message}`;
...
export async function fetchAndDisplayHeaderArt(genre) {
  try {
    ...
    // Handle Django response wrapper format
    if (responseData.status === "success" && responseData.data) {
      hideSkeletonLoaders();
      displayHeaderArt(responseData.data);
    } else {
      throw new Error(responseData.message || "Failed to fetch header art");
    }
  } catch (error) {
    console.error("Error fetching header art:", error);
    // Display user-visible error feedback
    showHeaderArtError(error.message);
  }
}
