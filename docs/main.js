...
=======
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        fetch(filename)
          ...
          .then((data) => {
            ...
            if (
              ...
              Object.keys(data[0].additional_sources).length > 0
            ) {
      ...
      function displaySources(cardBody, track) {
        ...
        // Incorporate main source and additional_sources into a unified object
        const allSources = { ...track.additional_sources, [track.source]: track.link };
    
        // Iterate over each source in allSources
        Object.keys(allSources).forEach((source) => {
            const sourceLink = allSources[source]; // Get the link from the sources object
            const sourceIcon = document.createElement("img");
            const linkElement = document.createElement("a"); // Create an anchor tag to wrap the image
            linkElement.href = sourceLink; // Set the hyperlink to the source's URL
            ...
            switch (source) {
                case "soundcloud":
                    ...
                    break;
                case "spotify":
                    sourceIcon.src = "./images/logos/spotify_logo.png";
                    sourceIcon.alt = "Spotify";
                    break;
                case "apple_music":
                    sourceIcon.src = "./images/logos/apple_music_logo.png";
                    sourceIcon.alt = "Apple Music";
                    break;
                // Add more cases as needed
            }
            linkElement.appendChild(sourceIcon); // Add the image to the anchor tag
            sourcesContainer.appendChild(linkElement); // Add the anchor tag to the sources container
        });
